...
## Group 1 — all four accounts verified (SQL simulation of the fixed query, FY2026)

All four are debit-normal (Cash and Equivalent → asset class) and each has exactly one
`OPENING_BALANCE` entry (no double-reset ambiguity):

| Account | Opening | Debit Carried | Credit Carried | Ending Balance |
|---|---:|---:|---:|---:|
| 110100 — البنك التجاري الدولى (CIB) | 11,118,958.00 | 99,223,724.87 | 107,717,263.97 | **2,625,418.90** |
| 110300 — بنك أبو ظبي الإسلامي | 4,091,279.00 | 24,155,018.95 | 22,255,150.27 | **5,991,147.68** |
| 110200 — بيت التمويل الكويتي | 2,275,235.00 | 26,785,054.50 | 29,038,982.01 | **21,307.49** |
| 111010 — النقدية (Cash) | 718,513.00 | 49,353,888.00 | 51,333,566.42 | **-1,261,165.42** |

110100, 110300, 110200 look healthy. 111010 comes out negative — investigated below; it's a real,
separate data issue, not a fix regression. GL 1211001's fallback branch was additionally confirmed
live in production (post-deploy re-export): header returned to 26,833,995 / 10,038,586 / 16,795,409,
matching this same simulation methodology exactly.

**Caveat:** these four are SQL-level simulations of the fixed handler's query shape, not live API
calls (except 1211001, confirmed live). Re-pull 111010, 110300, 110200 through the actual UI/export
to close the loop.

## GL 111010 (Cash) negative balance — root cause found, not a code bug

Walked the account's full FY2026 running balance chronologically from the 718,513 opening. It
stays healthy all year, reaching **3,693,817.50** by 2026-07-12. Then a single transaction:

```
ACCTG_TRANS_ID 17688 | 2026-07-22 | OUTGOING_PAYMENT | Credit | 5,162,482.92
"إيداع مبلغ 103,852 دولار بسعر 49.71ج من الخزينة الي حساب الشركة"
(Deposit of $103,852 at rate 49.71 EGP, from the cash treasury into the company's bank account)
```

drops the balance straight to -1,468,665.42, and it never fully recovers (two small incoming
payments later in the year bring it back only to -1,261,165.42, the final FY2026 figure).

**The real problem: this transaction is dated in the future.** 2026-07-22 is after the date this
was investigated (2026-07-13), yet it's already marked `IS_POSTED = 'Y'` and counted in the
running balance. Almost certainly either a wrong date entered on a real transaction, or a
scheduled/planned transfer that got marked posted before it actually occurred. This is a **ledger
data-entry issue**, unrelated to `GetGlAccountTransactionDetails.cs` — the fix is correctly
reporting what the ledger says. It would also already show up in Trial Balance's own top-level
grid for the same GL account/period, since Trial Balance bounds by the same period `ThruDate`
(2027-01-01 for FY2026), which is also past 2026-07-22.

**Action needed (outside this codebase):** flag `ACCTG_TRANS_ID 17688` to whoever manages the
cash/treasury account — either correct its date or un-post it until the transfer actually
happens. Once resolved, GL 111010's FY2026 ending balance should land back in positive territory.
