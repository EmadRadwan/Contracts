...
public class GetCustomersLov
{
    public class CustomersEnvelop
    {
        public List<PartyFromPartyIdDto> Parties { get; set; }
        public int PartyCount { get; set; }
    }

    public class Query : IRequest<Result<CustomersEnvelop>>
    {
        public PartyLovParams? Params { get; set; }
    }

    public class Handler : IRequestHandler<Query, Result<CustomersEnvelop>>
    {
        ...
        public async Task<Result<CustomersEnvelop>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(request.Params.SearchTerm))
            {
                ...
                // Check if the search term is a numeric value (customer number or phone number)
                if (int.TryParse(lowerCaseSearchTerm, out var customerNumber))
                    // Handle search by customer number
                    query = queryByPhone.Where(p =>
                        p.FromPartyPhone != null && p.FromPartyPhone.Contains(customerNumber.ToString()));
                else
            ...
            var PartyEnvelop = new CustomersEnvelop
            {
                Parties = Partys,
            ...
            return Result<CustomersEnvelop>.Success(PartyEnvelop);
        }
    }
}