    ...
    public class Handler : IRequestHandler<Query, Result<PartyDto>>
    {
        ...
        public async Task<Result<PartyDto>> Handle(Query request, CancellationToken cancellationToken)
        {
            var result = await (
                    ...
                    join pgaLoan in _context.PartyGlAccounts on new
                            { Org = "Company", P = prty.PartyId, R = "EMPLOYEE", T = "ACCOUNTS_RECEIVABLE" }
                        equals new
                        {
                            Org = pgaLoan.OrganizationPartyId, P = pgaLoan.PartyId, R = pgaLoan.RoleTypeId,
                            T = pgaLoan.GlAccountTypeId
                        } into pgaLoanGroup
                    from pgaLoan in pgaLoanGroup.DefaultIfEmpty()
                    join glaLoan in _context.GlAccounts on pgaLoan.GlAccountId equals glaLoan.GlAccountId into
                        glaLoanGroup
                    from glaLoan in glaLoanGroup.DefaultIfEmpty()
                    join pgaAccrued in _context.PartyGlAccounts on new
                            { Org = "Company", P = prty.PartyId, R = "EMPLOYEE", T = "ACCOUNTS_PAYABLE" }
                        equals new
                        {
                            Org = pgaAccrued.OrganizationPartyId, P = pgaAccrued.PartyId, R = pgaAccrued.RoleTypeId,
                            T = pgaAccrued.GlAccountTypeId
                        } into pgaAccruedGroup
                    from pgaAccrued in pgaAccruedGroup.DefaultIfEmpty()
                    join glaAccrued in _context.GlAccounts on pgaAccrued.GlAccountId equals glaAccrued.GlAccountId into
                        glaAccruedGroup
