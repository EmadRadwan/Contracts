...
public class CreateBroker
{
    ...
    public class Handler : IRequestHandler<Command, Result<PartyDto2>>
    {
        ...
        public async Task<Result<PartyDto2>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var roleTypeBroker = await _context.RoleTypes.SingleOrDefaultAsync(
                x => x.RoleTypeId == "BROKER", cancellationToken);
            ...
            if (!result)
            {
                ...
                return Result<PartyDto2>.Failure("Failed to create Broker");
            }
            ...
            {
                PartyId = newPartyId,
                Description = request.PartyDto.GroupName + " ( BROKER )",
