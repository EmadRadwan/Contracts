...
namespace Application.Facilities
{
    public class ReceiveInventoryFromPurchaseOrder
    {
        ...
        public class Handler : IRequestHandler<Command, Result<ReceiveInventoryResult>>
        {
            ...
            public Handler(DataContext context, IMediator mediator, ILogger<Handler> logger, IShipmentService shipmentService)
            {
                _context = context;
                _mediator = mediator;
                _logger = logger;
            ...
            public async Task<Result<ReceiveInventoryResult>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    if (purchaseOrder == null)
                    {
                        ...
                        return Result<ReceiveInventoryResult>.Failure($"Purchase Order with ID {request.OrderId} not found.");
                    }
                    if (!string.Equals(purchaseOrder.OrderTypeId, "PURCHASE_ORDER", StringComparison.OrdinalIgnoreCase))
                    {
                        _logger.LogWarning("OrderTypeId for Purchase Order ID {PurchaseOrderId} is not 'PURCHASE_ORDER'.", request.OrderId);
                        return Result<ReceiveInventoryResult>.Failure($"OrderTypeId for Purchase Order ID {request.OrderId} is invalid.");
                    }
                    ...
                    {
                        var quickReceiveResult = await _shipmentService.QuickReceivePurchaseOrder(request.OrderId, request.FacilityId);
                    ...
                    var receivedItemsDto = await ConstructReceiveInventoryItemsDto(request.OrderId, request.FacilityId, orderItems, cancellationToken);
                    ...
                    var result = await _mediator.Send(new ReceiveInventoryProducts.Command { ReceivedItems = receivedItemsDto }, cancellationToken);
                    if (!result.IsSuccess)
                    {
                        _logger.LogError("Failed to receive inventory for OrderId: {OrderId}. Error: {Error}", request.OrderId, result.Error);
                        if (ownsTransaction)
                        {
                            await transaction.RollbackAsync(cancellationToken);
                        }
                ...
                {
                    _logger.LogError(ex, "An error occurred while processing ReceiveInventoryFromPurchaseOrder for OrderId: {OrderId}.", request.OrderId);
                    if (ownsTransaction)
                    {
                        await transaction.RollbackAsync(cancellationToken);
                    }
                    return Result<ReceiveInventoryResult>.Failure($"An error occurred while receiving inventory: {ex.Message}");
                }
                finally
                {
            ...
            private async Task<List<OrderItem>> GetOrderItemsForPurchaseOrder(string orderId, CancellationToken cancellationToken)
            {
