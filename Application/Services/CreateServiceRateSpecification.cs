...
public class CreateServiceSpecification
{
    ...
    public class Handler : IRequestHandler<Command, Result<ServiceSpecificationDto>>
    {
        ...
        public async Task<Result<ServiceSpecificationDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var existingServiceSpecification = await _context.ServiceSpecifications
                .SingleOrDefaultAsync(x => x.ProductId == request.ServiceSpecificationDto.ProductId.ProductId
                                           && x.MakeId == request.ServiceSpecificationDto.MakeId
            ...
            var newServiceSpecification =
                await _vehicleService.CreateServiceSpecification(request.ServiceSpecificationDto);


            var result = await _context.SaveChangesAsync(cancellationToken) > 0;

            if (!result) return Result<ServiceSpecificationDto>.Failure("Failed to create ServiceSpecification");
