...
public class CreateServiceRate
{
    ...
    public class Handler : IRequestHandler<Command, Result<ServiceRateDto>>
    {
        ...
        public async Task<Result<ServiceRateDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var newServiceRate = await _vehicleService.CreateServiceRate(request.ServiceRateDto);


            var result = await _context.SaveChangesAsync(cancellationToken) > 0;

            if (!result) return Result<ServiceRateDto>.Failure("Failed to create ServiceRate");
