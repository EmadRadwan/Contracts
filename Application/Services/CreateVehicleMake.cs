...
public class CreateVehicleMake
{
    ...
    public class Handler : IRequestHandler<Command, Result<VehicleMakeDto>>
    {
        ...
        public async Task<Result<VehicleMakeDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var newVehicleMake = await _vehicleService.CreateVehicleMake(request.VehicleMakeDto);


            var result = await _context.SaveChangesAsync(cancellationToken) > 0;

            if (!result) return Result<VehicleMakeDto>.Failure("Failed to create VehicleMake");
