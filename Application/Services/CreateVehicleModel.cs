    ...
    public class Handler : IRequestHandler<Command, Result<VehicleModelDto>>
    {
        ...
        public async Task<Result<VehicleModelDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            if (existingVehicleModel != null)
                return Result<VehicleModelDto>.Failure("Vehicle Model with Same Name Already Exists");

            var newVehicleModel = await _vehicleService.CreateVehicleModel(request.VehicleModelDto);


            var result = await _context.SaveChangesAsync(cancellationToken) > 0;

            if (!result) return Result<VehicleModelDto>.Failure("Failed to create Vehicle Model");
