    ...
    public class Handler : IRequestHandler<Query, Result<OrderDto2>>
    {
        ...
        public async Task<Result<OrderDto2>> Handle(Query request, CancellationToken cancellationToken)
        {
            var query = from ord in _context.OrderHeaders
                ...
                where ord.OrderId == request.OrderId && orole.RoleTypeId == "SUPPLIER"
            ...
            var orderItems = (from itm in _context.OrderItems
                ...
                join supp in _context.SupplierProducts on prd.ProductId equals supp.ProductId
                ...
                select new OrderItemDto
                {
                    ...
                    ProductId = new ProductLovDto
                    {
                        ...
                        LastPrice = supp.LastPrice,
                        AvailableFromDate = supp.AvailableFromDate,
                        AvailableThruDate = supp.AvailableThruDate
                    },
                    ProductName = prd.ProductName,
            ...
            // todo revisit adjustment data - Product mainly - that needs to be sent to client
