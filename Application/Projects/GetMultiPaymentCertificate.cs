...
public class GetMultiPaymentCertificate
{
    public class Query : IRequest<Result<MultiPaymentCertificateRecord>>
    {
        public string WorkEffortId { get; set; }
        public string Language { get; set; }
    }

    public class Handler : IRequestHandler<Query, Result<MultiPaymentCertificateRecord>>
    {
        ...
        public async Task<Result<MultiPaymentCertificateRecord>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var record = await (
                ...
                select new MultiPaymentCertificateRecord
                {
                    ...
                    Amount = _context.WorkEfforts
                        .Where(item => item.WorkEffortParentId == we.WorkEffortId
            ...
            if (record == null)
                return Result<MultiPaymentCertificateRecord>.Failure("Certificate not found");

            return Result<MultiPaymentCertificateRecord>.Success(record);
        }
    }
}
