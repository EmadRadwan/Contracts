...
{
    public class ListProjectCertificatesByDateRange
    {
        public class Query : IRequest<List<ProjectCertificateRecord>>
        {
        ...
        public class Handler : IRequestHandler<Query, List<ProjectCertificateRecord>>
        {
            ...
            public async Task<List<ProjectCertificateRecord>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                {
                    query = query.Where(p => p.EstimatedStartDate >= request.StartDate.Value);
                }

                if (request.EndDate.HasValue)
                {
                    query = query.Where(p => p.EstimatedStartDate <= request.EndDate.Value);
                }
                ...
                {
                    record.CertificateCategoryDescription = GetCertificateCategoryDescription(record.CertificateCategory);
                }
