...
public class CompleteSalesOrderPayments
{
    ...
    public class Handler : IRequestHandler<Command, Result<PaymentsDto>>
    {
        ...
        public async Task<Result<PaymentsDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            if (request.PaymentsDto.Payments != null)
            {
                ...
                foreach (var updatedPayment in request.PaymentsDto.Payments)
                {
                    ...
                    if (savedPayment != null)
                    {
                        ...
                        else
                        {
                            ...
                            savedPayment.StatusId = "PMNT_RECEIVED";

                            // get payment application sequence from sequence value item
                            var paymentApplicationSequenceRecord = await _context.SequenceValueItems
                                .Where(x => x.SeqName == "PaymentApplication").SingleOrDefaultAsync();
                            var newPaymentApplicationSequence = paymentApplicationSequenceRecord.SeqId + 1;
                            paymentApplicationSequenceRecord.SeqId = newPaymentApplicationSequence;
            ...
            if (!result)
            {
                ...
                return Result<PaymentsDto>.Failure("Failed to complete payments");
            }
...
//todo: This needs to be handled via the Accounting module