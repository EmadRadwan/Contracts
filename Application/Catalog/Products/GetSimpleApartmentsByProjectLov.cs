...
public class GetSimpleApartmentsByProjectLov
{
    ...
    public class ApartmentLovDto
    {
        ...
        public string ProjectName { get; set; } = string.Empty;         // from Projects table
    ...
    public class Query : IRequest<Result<ApartmentsEnvelope>>
    {
        ...
        public string ProjectId { get; set; } = string.Empty;   // ← added / required for filtering
    }
    ...
    public class Handler : IRequestHandler<Query, Result<ApartmentsEnvelope>>
    {
        ...
        public async Task<Result<ApartmentsEnvelope>> Handle(Query request, CancellationToken ct)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("ProjectId is required but was not provided");
                    return Result<ApartmentsEnvelope>.Failure("Project ID is required.");
                }
                ...
                // -----------------------------------------------------------------
                // 3. Base query – filter by project + type + optional search
                // -----------------------------------------------------------------
                ...
                // -----------------------------------------------------------------
                // 4. Fetch paged products
                // -----------------------------------------------------------------
                var products = await baseQuery
                    .OrderBy(p => p.ProductId)           // initial sort for stable paging
                    .Skip(request.Params.Skip)
                    .Take(request.Params.PageSize)
                    .ToListAsync(ct);

                // Apply natural sort after materialization (if needed)
                products = products
                    .OrderBy(p => p.ProductId, Comparer<string>.Create(NaturalCompare))
                    .ToList();

                // -----------------------------------------------------------------
                // 5. Floor number Arabic mapping
                // -----------------------------------------------------------------
            ...
            {
                _logger.LogError(ex, "Failed to retrieve apartments for project {ProjectId}", request.ProjectId);
