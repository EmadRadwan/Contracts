...
{
    /// <inheritdoc />
    public partial class FixInvoiceRecordsViewIsPositiveAmount : Migration
    {
        /// <inheritdoc />
...
SELECT
    ...
    -- Conditional aggregation replaces correlated subqueries so MySQL can join
    -- INVOICE_ITEM_TYPE once per row and apply IsPositiveAmount sign correctly.
    ROUND(
        CASE
            -- At least one PINV_CERTIFICATE_ITEM line exists on this invoice
            WHEN MAX(CASE WHEN ii.INVOICE_ITEM_TYPE_ID = 'PINV_CERTIFICATE_ITEM' THEN 1 ELSE 0 END) = 1
            THEN
                -- Certificate lines (signed)
                COALESCE(SUM(
            ...
            -- No certificate lines: signed sum of all items
            ELSE
                COALESCE(SUM(
...
");
        ...
        {
            // Restore the original view (correlated subqueries, no IsPositiveAmount sign)
            migrationBuilder.Sql(@"
CREATE OR REPLACE VIEW InvoiceRecords AS
SELECT
    inv.INVOICE_ID AS InvoiceId,
    ...
    ROUND(
        CASE
            WHEN EXISTS (
                SELECT 1 FROM INVOICE_ITEM ii_cert
                WHERE ii_cert.INVOICE_ID = inv.INVOICE_ID
                  AND ii_cert.INVOICE_ITEM_TYPE_ID = 'PINV_CERTIFICATE_ITEM'
            )
            THEN
                COALESCE((SELECT SUM(ii_cert.QUANTITY * ROUND(ii_cert.AMOUNT, 5))
                          FROM INVOICE_ITEM ii_cert
                          WHERE ii_cert.INVOICE_ID = inv.INVOICE_ID
                            AND ii_cert.INVOICE_ITEM_TYPE_ID = 'PINV_CERTIFICATE_ITEM'), 0.00)
                -
                COALESCE((SELECT SUM(ii_other.QUANTITY * ROUND(ii_other.AMOUNT, 5))
                          FROM INVOICE_ITEM ii_other
                          WHERE ii_other.INVOICE_ID = inv.INVOICE_ID
                            AND ii_other.INVOICE_ITEM_TYPE_ID != 'PINV_CERTIFICATE_ITEM'), 0.00)
            ELSE
                COALESCE((SELECT SUM(ii.QUANTITY * ROUND(ii.AMOUNT, 5))
                          FROM INVOICE_ITEM ii
                          WHERE ii.INVOICE_ID = inv.INVOICE_ID), 0.00)
        END,
        2
    ) AS Total
FROM INVOICE inv
