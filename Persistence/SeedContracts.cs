                ...
                if (projectName == "الصحراوى 10.5 فدان")
                {
                    foreach (var subProjectName in subProjects)
                    {
                        ...
                        var subProject = new WorkEffort
                        {
                            ...
                            GlAccountId = glAccountMapping.ContainsKey(projectName)
                                ? glAccountMapping[projectName]
                                : null
                        };
        ...
        if (!context.PartyRoles.Any())
        {
            ...
            foreach (var party in parties)
            {
                if (party.MainRole == "CUSTOMER")
                {
                    ...
                    {
                        "BILL_TO_CUSTOMER", "CONTACT", "PLACING_CUSTOMER", "SHIP_TO_CUSTOMER", "END_USER_CUSTOMER",
                        "CUSTOMER"
                    };
                ...
                else if (party.MainRole == "SUPPLIER")
                {
                    string[] supplierRoles =
                        { "SUPPLIER_AGENT", "SHIP_FROM_VENDOR", "BILL_FROM_VENDOR", "SUPPLIER", "ACCOUNT" };
                ...
                else if (party.PartyId == "Company")
                {
                    string[] companyRoles =
                    {
                        "CARRIER", "INTERNAL_ORGANIZATIO", "BILL_TO_CUSTOMER", "BILL_FROM_VENDOR", "ACCOUNT", "_NA_"
                    };
                ...
                // REFACTOR: Added check for CONTRACTOR role to assign only the CONTRACTOR role, ensuring consistency with other single-role assignments like TAX_AUTHORITY and simplifying role management for contractors.
                else if (party.MainRole == "CONTRACTOR")
                {
                    string[] contractorRoles =
                    {
                        "SUPPLIER_AGENT", "SHIP_FROM_VENDOR", "BILL_FROM_VENDOR", "SUPPLIER", "ACCOUNT", "CONTRACTOR"
                    };
                    foreach (var role in contractorRoles)
                    {
                        context.PartyRoles.Add(new PartyRole
                        {
                            PartyId = party.PartyId,
