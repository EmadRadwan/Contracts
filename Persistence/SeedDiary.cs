...
public class SeedDiary
{
        ...
        UserManager<AppUserLogin> userManager, RoleManager<ApplicationRole> roleManager)
    {
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_category_types_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_categories_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/products_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_prices_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_assocs_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_category_members_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_facilities_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        if (!context.BillingAccounts.Any())
        {
            ...
            foreach (var party in parties.Where(p => p.MainRole == "CUSTOMER"))
            {
                ...
                {
                    BillingAccountId = Guid.NewGuid().ToString(), // Generate unique ID
        ...
        if (!context.ContactMeches.Any())
        {
            ...
            foreach (var party in context.Parties.ToList())
            {
                ...
                if (party.MainRole == "EMPLOYEE" || party.MainRole == "_NA_")
                {
                    continue;
                }
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/tax_authority_categories_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/fixed_asset_types_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/fixed_assets_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workefforts_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/cost_components_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workeffort_assocs_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workeffort_good_standards_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workeffort_cost_calcs_dairy.json");
            var jsonData = File.ReadAllText(path);
        ...
        if (!context.InventoryItems.Any())
        {
            ...
            foreach (var product in rawMaterials)
            {
                ...
                var selectedSuppliers = shuffledSuppliers
                    .Take(2)
                    .ToList();

                foreach (var supplier in selectedSuppliers)
                {
                    var baseQuantity = faker.Random.Int(190, 210); // Ensure quantities are integers

                    // Fetching the unit cost from ProductPrices table
                    var unitCost = context.ProductPrices
                        .Where(pp => pp.ProductId == product.ProductId && pp.ProductPriceTypeId == "DEFAULT_PRICE")
                        .Select(pp => pp.Price)
                        .FirstOrDefault();
                    ...
                    var inventoryItem = new InventoryItem
                    {
                        ...
                        UnitCost = unitCost ?? faker.Random.Decimal(8, 10),
                        CurrencyUomId = "EGP",
                        CreatedStamp = nowDateTime,
                        LastUpdatedStamp = nowDateTime
                    };
                    ...
                    // Increment the ID counters for the next record
                    inventoryItemIdStart++;
                    inventoryItemDetailSeqIdStart++;
                }
            }

            // Fetching products where ProductTypeId is 'FINISHED_GOOD'
...
//---------------------------------Product Promo---------------------------------
            ...
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_headers.json");
                var jsonData = File.ReadAllText(path);

                var orderHeaders = JsonConvert.DeserializeObject<List<OrderHeader>>(jsonData);
                await context.OrderHeaders.AddRangeAsync(orderHeaders);
                await context.SaveChangesAsync();
            }
            ...
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_items_dairy.json");
                var jsonData = File.ReadAllText(path);

                var orderItems = JsonConvert.DeserializeObject<List<OrderItem>>(jsonData);
                await context.OrderItems.AddRangeAsync(orderItems);
                await context.SaveChangesAsync();
            }*/

            // Order Roles
            /*if (!context.OrderRoles.Any())
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_roles.json");
                var jsonData = File.ReadAllText(path);

                var orderRoles = JsonConvert.DeserializeObject<List<OrderRole>>(jsonData);
                await context.OrderRoles.AddRangeAsync(orderRoles);
                await context.SaveChangesAsync();
            }


            // Order Item Ship Groups
            if (!context.OrderItemShipGroups.Any())
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_item_ship_groups.json");
                var jsonData = File.ReadAllText(path);

                var orderItemShipGroups = JsonConvert.DeserializeObject<List<OrderItemShipGroup>>(jsonData);
                await context.OrderItemShipGroups.AddRangeAsync(orderItemShipGroups);
                await context.SaveChangesAsync();
            }*/

            /*if (!context.OrderAdjustments.Any())
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_adjustments.json");
                var jsonData = File.ReadAllText(path);

                var orderAdjustments = JsonConvert.DeserializeObject<List<OrderAdjustment>>(jsonData);
                await context.OrderAdjustments.AddRangeAsync(orderAdjustments);
                await context.SaveChangesAsync();
            }*/

            /*if (!context.OrderPaymentPreferences.Any())
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_payment_preferences.json");
                var jsonData = File.ReadAllText(path);

                var orderPaymentPreferences = JsonConvert.DeserializeObject<List<OrderPaymentPreference>>(jsonData);
                await context.OrderPaymentPreferences.AddRangeAsync(orderPaymentPreferences);
                await context.SaveChangesAsync();
            }

            if (!context.OrderStatuses.Any())
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/order_statuses.json");
                var jsonData = File.ReadAllText(path);

                var orderStatuses = JsonConvert.DeserializeObject<List<OrderStatus>>(jsonData);
                await context.OrderStatuses.AddRangeAsync(orderStatuses);
                await context.SaveChangesAsync();
            }*/

            // Automate OrderContactMech creation
            /*if (!context.OrderContactMeches.Any())
            {
                var orders = new List<(string OrderId, string ContactMechPurposeTypeId, string PartyId)>
                {
                    ("Demo1002", "BILLING_LOCATION", "SomePartyIdForBilling"),
                    ("Demo1002", "ORDER_EMAIL", "SomePartyIdForEmail"),
                    ("Demo1002", "SHIPPING_LOCATION", "SomePartyIdForShipping")
                };

                foreach (var (orderId, purposeTypeId, partyId) in orders)
                {
                    if (contactMechIds.TryGetValue((partyId, purposeTypeId), out var contactMechId))
                    {
                        await context.OrderContactMeches.AddAsync(new OrderContactMech
                        {
                            OrderId = orderId,
            ...
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/shipments.json");
                var jsonData = File.ReadAllText(path);

                var shipments = JsonConvert.DeserializeObject<List<Shipment>>(jsonData);
            ...
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/shipment_items_dairy.json");
                var jsonData = File.ReadAllText(path);

                var shipmentItems = JsonConvert.DeserializeObject<List<ShipmentItem>>(jsonData);
            ...
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/shipment_statuses.json");
                var jsonData = File.ReadAllText(path);

                var shipmentStatuses = JsonConvert.DeserializeObject<List<ShipmentStatus>>(jsonData);
                await context.ShipmentStatuses.AddRangeAsync(shipmentStatuses);
                await context.SaveChangesAsync();
            }
            */
            ...
            if (!context.Agreements.Any())
            {
                try
                {
                    // Fetch parties with the required roles
                    var partiesWithRoles = await context.Parties
                        .Where(p => context.PartyRoles.Any(pr => pr.PartyId == p.PartyId &&
                                                                 (pr.RoleTypeId == "CUSTOMER" ||
                                                                  pr.RoleTypeId == "SUPPLIER")))
                        .ToListAsync();

                    // Process each party and create related agreements based on roles
                    foreach (var party in partiesWithRoles)
                    {
                        var partyRoles = await context.PartyRoles
                            .Where(pr => pr.PartyId == party.PartyId)
                            .ToListAsync();

                        if (partyRoles.Any(pr => pr.RoleTypeId == "CUSTOMER"))
                        {
                            // Create a Sales Agreement
                            var salesAgreement = new Agreement
                            {
                                ...
                                Description = $"Sales Agreement for {party.Description}",
                                CreatedStamp = DateTime.Now,
                                LastUpdatedStamp = DateTime.Now
                            };

                            await context.Agreements.AddAsync(salesAgreement);

                            // Create Agreement Terms
                            var salesAgreementTerm = new AgreementTerm
                            {
                                AgreementTermId = salesAgreement.AgreementId,
                                AgreementId = salesAgreement.AgreementId,
                                TermTypeId = "FIN_PAYMENT_TERM",
                                TermValue = 1.0m,
                                TermDays = 30,
                                Description = "Sales test agreement",
                                CreatedStamp = DateTime.Now,
                                LastUpdatedStamp = DateTime.Now
                            };

                            await context.AgreementTerms.AddAsync(salesAgreementTerm);

                            // Create Agreement Items
                            var salesAgreementItem = new AgreementItem
                            {
                                AgreementId = salesAgreement.AgreementId,
                                AgreementItemSeqId = "00001",
                                AgreementItemTypeId = "AGREEMENT_PRICING_PR",
                                CurrencyUomId = "EGP",
                                AgreementText = "Price list in EGP",
                                CreatedStamp = DateTime.Now,
                                LastUpdatedStamp = DateTime.Now
                            };

                            await context.AgreementItems.AddAsync(salesAgreementItem);
                        }

                        if (partyRoles.Any(pr => pr.RoleTypeId == "SUPPLIER"))
                        {
                            // Create a Purchase Agreement
                            var purchaseAgreement = new Agreement
                            {
                                AgreementId = Guid.NewGuid().ToString(),
                                PartyIdFrom = "Company",
                                ...
                                Description = $"Purchase Agreement with {party.Description}",
                                CreatedStamp = DateTime.Now,
                                LastUpdatedStamp = DateTime.Now
                            };

                            await context.Agreements.AddAsync(purchaseAgreement);

                            // Create Agreement Terms for Purchase Agreement
                            var purchaseAgreementTerm = new AgreementTerm
                            {
                                AgreementTermId = purchaseAgreement.AgreementId,
                                AgreementId = purchaseAgreement.AgreementId,
                                TermTypeId = "FIN_PAYMENT_TERM",
                                TermValue = 1.0m,
                                TermDays = 30,
                                Description = "Purchase agreement term",
                                CreatedStamp = DateTime.Now,
                                LastUpdatedStamp = DateTime.Now
                            };

                            await context.AgreementTerms.AddAsync(purchaseAgreementTerm);
                        }
                    }
                ...
                {
                    // Log each party and role type during the failure
                    var entities = context.ChangeTracker.Entries<PartyRole>()
                    ...
                    {
                        Console.WriteLine($"Inserting PARTY_ID: {entity.PartyId}, ROLE_TYPE_ID: {entity.RoleTypeId}");
                    }
