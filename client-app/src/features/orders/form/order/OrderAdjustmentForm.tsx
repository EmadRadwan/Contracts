...
export default function OrderItemAdjustmentForm({
    ...
    const defaultOrderAdjustmentType = orderAdjustmentTypesData?.find(
        (x: OrderAdjustmentType) => x.description === "Discount"
    ...
    async function handleSubmitData(data: OrderAdjustment) {
        ...
        try {
            if (editMode === 2) {
                ...
                newOrderAdjustment = {
                    orderAdjustmentId: undefined, // Reducer generates key
                    amount: data.amount,
            ...
            console.log("DEBUG: Updated Adjustments:", JSON.stringify(updatedAdjustments, null, 2));

            // REFACTOR: Removed calculateTaxMutation call
            // Purpose: Ensure SalesOrderForm.tsx is the sole gate for tax API triggers
            // Why: Tax calculation is handled by SalesOrderForm.tsx useEffect on allAvailableAdjustments change
