...
// Improvement: Improves readability and reusability by separating cell rendering logic
const IssuedQuantityCell = ({ dataItem }) => {
...
export default function ProductionRunMaterialsList({
  ...
  const shouldUseMaterialsQuery = ["Created", "Scheduled", "Confirmed"].includes(
      currentStatusDescription || ""
  );
  ...
  const { data: productionRunMaterialsData } = useFetchProductionRunMaterialsQuery(productionRunId, {
  ...
  const { data: productionRunIssueMaterialsData } = useFetchIssueProductionRunDeclComponentsQuery(productionRunId, {
      ...
      <Fragment>
        ...
        {shouldUseMaterialsQuery && (
            <div className="div-container">
              <KendoGrid
                  ...
                  skip={page.skip}
                  take={page.take}
                  total={productionRunMaterialsData ? productionRunMaterialsData.length : 0}
                  pageable={true}
        ...
        {shouldUseIssueComponentsQuery && (
            <KendoGrid
                ...
                skip={page.skip}
                take={page.take}
                total={productionRunIssueMaterialsData ? productionRunIssueMaterialsData.length : 0}
                pageable={true}
                onPageChange={pageChange}
                resizable={true}
            >
              ...
              <Column
                  ...
                  width={100}
              />
              {/* REFACTOR: Added custom cell component to issuedQuantity column
             Purpose: Enables conditional styling based on comparison with estimatedQuantity
             Improvement: Allows visual highlighting without modifying the entire row rendering */}
              <Column
