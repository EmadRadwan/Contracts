...
import {QuoteItemAdjustmentsListMemo} from "./QuoteItemAdjustmentsList";
...
import {setSelectedQuoteItem, setUiQuoteItems} from "../../orders/slice/quoteItemsUiSlice";
...
export default function JobQuoteItemsList({
                                              quoteFormEditMode,
                                              quoteId,
                                          }: Props) {
    // state for the grid
    ...
    function handleSelectQuoteItem(quoteItemId: string) {
        ...
        if (selectedQuoteItem?.quoteItemProduct) {
            quoteItemToDisplay = {...selectedQuoteItem, productId: selectedQuoteItem?.quoteItemProduct};
        } else {
            quoteItemToDisplay = {...selectedQuoteItem, productId: selectedQuoteItem?.productLov};
        }
    ...
    const remove = (dataItem: QuoteItem) => {
        ...
        localQuoteItems = localQuoteItems?.map((item: QuoteItem) => {
            if (newRelatedQuoteItems?.some((quoteItem: QuoteItem) => quoteItem.quoteItemSeqId === item.quoteItemSeqId)) {
                return newRelatedQuoteItems?.find((quoteItem: QuoteItem) => quoteItem.quoteItemSeqId === item.quoteItemSeqId);
            } else {
                return item;
            }
        ...
        const relatedQuoteAdjustments = uiQuoteAdjustments?.filter((item: QuoteAdjustment) => {
            return localQuoteItems?.some(
                (quoteItem: QuoteItem) =>
                    quoteItem.quoteItemSeqId === item.quoteItemSeqId && quoteItem.isProductDeleted
            );
        });
        // mark related quote adjustments as deleted
        const newRelatedQuoteAdjustments = relatedQuoteAdjustments?.map((item: QuoteAdjustment) => {
    ...
    return (
        <>
            ...
            {showItemAdjustmentList && (
                <ModalContainer show={showItemAdjustmentList} onClose={memoizedOnClose} width={500}>
