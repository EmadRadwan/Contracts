...
import {productsEndpoints, quoteTaxAdjustmentsEndpoints,} from "../../../app/store/apis";
...
export default function useQuoteItem({
    ...
    const isNewServiceRateAndSpecificationAdded = useAppSelector(
        (state) => state.quoteUi.isNewServiceRateAndSpecificationAdded,
    );
    ...
    /* let { data: quoteItemProduct } = useFetchQuoteItemProductQuery(quoteItem, {
      skip,
    });*/
    ...
    async function createOrUpdateQuoteItem(data: any): Promise<QuoteItem> {
        ...
        if (editMode === 2) {
            ...
            newQuoteItem = {
                ...
                quoteUnitListPrice: isNewServiceRateAndSpecificationAdded
