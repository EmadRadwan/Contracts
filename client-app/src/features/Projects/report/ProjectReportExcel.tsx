...
import { useLazyFetchProjectReportQuery } from "../../../app/store/apis/projectsApi";
...
export const ProjectReportExcel: React.FC<ProjectReportExcelProps> = ({
    ...
    const generateExcel = useCallback(async (data: ProjectReportDto) => {
        ...
        // Headers
        const expHeaders = [
            'رقم الشهادة', 'اسم الطرف', 'الدور', 'المنتج/الخدمة', 'التاريخ',
        ...
        data.expenses.forEach((exp, idx) => {
            ...
            // Format monetary columns (I to O)
            for (let i = 9; i <= 15; i++) {
        ...
        // Column widths
        const expColWidths = [18, 32, 14, 28, 14, 22, 38, 45, 12, 16, 18, 16, 16, 16, 20];
        ...
        wsRev.mergeCells(`A${revRow}:J${revRow}`);
        wsRev.getRow(revRow).height = 45;

        revRow += 2;

        const revHeaders = [
            'رقم الدفعة', 'الوحدة', 'العميل', 'الفئة', 'المجدول',
            'المحصل', 'المتبقي', 'الحالة', 'شريحة التأخير', 'تاريخ الاستحقاق'
        ];
        ...
        const revWidths = [16, 14, 32, 20, 18, 18, 18, 16, 24, 16];
