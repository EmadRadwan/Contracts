...
export default function MultiPaymentItemsByGlAccountFilterExcel() {
    ...
    const generateExcel = useCallback(async (data: any[], glAccountName: string) => {
        ...
        const title = `‫بنود مستندات الصرف - حساب: ${glAccountName} (${startDate?.format("DD/MM/YYYY") || ""} - ${endDate?.format("DD/MM/YYYY") || ""})`;
        ...
        const headers = [
            ...
            getTranslatedLabel("projects.multiPaymentCertificate.items.description", "الوصف"),
            ...
            getTranslatedLabel("projects.multiPaymentCertificate.items.estimatedStartDate", "التاريخ"),
            getTranslatedLabel("projects.multiPaymentCertificate.items.serviceName", "الخدمة"),
            ...
            getTranslatedLabel("projects.certificate.form.subProject", "بنود المشروع"),
        ...
        ws.columns = [
            { width: 15 }, { width: 15 }, { width: 30 }, { width: 25 },
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const filtered = (allItems || []).filter(
                (item: any) => item.glAccountId === selectedGlAccount.glAccountId
            );
            ...
            const buffer = await generateExcel(filtered, selectedGlAccount.accountName || selectedGlAccount.glAccountId);
            if (buffer) {
