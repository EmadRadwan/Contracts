...
const PartyFinancialHistory: React.FC<Props> = ({partyId, partyName}) => {
    ...
    const ledgerItems = useMemo((): LedgerRow[] => {
        ...
        interface UnifiedTransaction {
            ...
            transactionTypeId?: string;     // ← new – for apartment sales distinction
        ...
        // ───────────────────────────────────────────────
        // NEW: Apartment Sales (only debit side matters for balance)
        // ───────────────────────────────────────────────
        (data.apartmentSalePostings || []).forEach((sale) => {
            if (!sale.transactionDate || sale.amount <= 0) return;

            // We only care about receivable increase → دائن column
            // Ignore credit side (revenue or AR offset) for balance purposes
            const isChequeType = sale.transactionTypeId === "APARTMENT_SALE_CHEQUE";

            transactions.push({
                date: sale.transactionDate,
                type: "apartmentSale",
                id: sale.salesRequestId || sale.transactionId,
                description: sale.description ||
                    (isChequeType
                        ? `بيع شقة بشيكات مؤجلة - طلب ${sale.salesRequestId || "غير معروف"}`
                        : `بيع شقة بالتقسيط - طلب ${sale.salesRequestId || "غير معروف"}`),
                grossAmount: sale.amount,
                transactionTypeId: sale.transactionTypeId,
                debitCreditFlag: sale.debitCreditFlag,
                salesRequestId: sale.salesRequestId,
            });
        });
        ...
        transactions.forEach((t) => {
            ...
            if (t.type === "invoice") {
                ...
                if (flag === "D") {
                    madin = t.grossAmount;
                    periodChange = +t.grossAmount;    // receivable increase
                } else if (flag === "C") {
                    dain = t.grossAmount;
        ...
        return (
            ...
            >
                ...
                >
                    <Tab
                        label={getTranslatedLabel('party.financial.history.tabs.invoicesPayments', 'Invoices & Payments')}/>
                    <Tab
                        label={getTranslatedLabel('party.financial.history.tabs.unappliedInvoices', 'Unapplied Invoices')}/>
                    <Tab
                        label={getTranslatedLabel('party.financial.history.tabs.unappliedPayments', 'Unapplied Payments')}/>
