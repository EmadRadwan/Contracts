...
export default function IncomeStatementGlAccountTransactionsModal({
    ...
    const localizationKey = 'accounting.orgGL.reports.income-statement.transactions';
    ...
    const { data, isLoading, isFetching } = useFetchIncomeStatementGlAccountTransactionDetailsQuery(
        ...
        {
            skip: !organizationPartyId || !glAccountId || (!fromDate && !thruDate && selectedMonth === undefined)
        }
    );
                        ...
                        <Grid item xs={12}>
                            <Box sx={{
                                ...
                                <Typography variant="body1">
                                    <strong>Opening Balance:</strong> {data.openingBalance?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Typography>
                                <Typography variant="body1">
                                    <strong>Posted Debits:</strong> {data.postedDebits?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Typography>
                                <Typography variant="body1">
                                    <strong>Posted Credits:</strong> {data.postedCredits?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Typography>
                                <Typography variant="body1" sx={{ fontWeight: 'bold', mt: 1 }}>
                                    <strong>Ending Balance:</strong> {data.endingBalance?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Typography>
                            </Box>
                        </Grid>
