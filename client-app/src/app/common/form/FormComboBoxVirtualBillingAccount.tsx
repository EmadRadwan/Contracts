...
export const FormComboBoxVirtualBillingAccount = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    interface Item {
        contactMechName: string;
        billingAccountId: string;
    }

    const textField = "contactMechName";
    const keyField = "billingAccountId";
    const emptyItem: Item = {[textField]: "loading ...", billingAccountId: "0"};
    const pageSize = 10;
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        agent.BillingAccounts.getBillingAccountsLov(params)
            .then((json) => {
                if (json) {
                    const total = json.billingAccountCount;
                    const items: Item[] = [];
                    json.billingAccounts.forEach((element: any, index: any) => {
                        const {billingAccountId, contactMechName} = element;
                        const item: Item = {
                            [keyField]: billingAccountId,
                            [textField]: contactMechName,
                        };
                        items.push(item);
    ...
    const onChangeHandler = React.useCallback(
        (event) => {
            /*if (event.value === null) {
                ...
                dispatch(setCustomerId(event.value.billingAccountId));
            }*/
