...
export const FormMultiColumnComboBoxVirtualPaymentApplication = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    const columns = [
        ...
        {field: "partyId", header: "From Party"}
    ];
    ...
    const textField = "partyId";
    const keyField = "paymentId";
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        if (["PURCHASE_INVOICE", "PAYROL_INVOICE", "COMMISSION_INVOICE", "CUST_RTN_INVOICE"].includes(selectedInvoice!.invoiceTypeId!)) {
            params.append('partyId', selectedInvoice?.partyIdFrom?.fromPartyId!);
        } else {
            params.append('partyId', selectedInvoice?.partyId?.fromPartyId!);
        }

        if (filter) params.append('searchTerm', filter);
        agent.Accounting.getPaymentApplications(params)
            .then((json) => {
                if (json) {
                    const total = json.paymentsCount;
                    const items: Item[] = [];
                    json.payments.forEach((element: any, index: any) => {
                        const {
                            paymentId,
                            partyIdFrom
                        } = element;
                        const item: Item = {
                            [keyField]: paymentId,
                            [textField]: partyIdFrom.fromPartyName!
                        };
                        items.push(item);
