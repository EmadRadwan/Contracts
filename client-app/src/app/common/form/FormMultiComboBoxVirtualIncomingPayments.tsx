...
interface ExtendedFieldRenderProps extends FieldRenderProps {
    // REFACTOR REMARK: new prop for width (optional), defaults to "200px" if not provided
    columnWidth?: string;
} 
export const FormMultiComboBoxVirtualIncomingPayments = (
    ...
    const columns = [
        {field: "paymentId", header: "Payment", width: columnWidth},
        {field: "amount", header: "Amount", width: columnWidth},
        {field: "paymentTypeDescription", header: "Payment Type", width: columnWidth}
    ];
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        agent.Accounting.listIncomingPayments(params).then((json) => {
            if (json) {
                const total = json.paymentCount;
                const items: Item[] = [];
                json.payments.forEach((element: any, index: any) => {
                    const {paymentId, amount, paymentTypeDescription, lovText} = element;
                    const item: Item = {
                        [keyField]: paymentId,
                        [textField]: lovText,
