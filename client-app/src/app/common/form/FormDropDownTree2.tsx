...
export const FormDropDownTree2 = (fieldRenderProps: FieldRenderProps & DropDownTreeProps) => {
    const {
        validationMessage,
        touched,
        label,
    ...
    const onChangeHandler = React.useCallback(
        (event) => {
            if (event.value && event.value.items) {
                return
            } else {
    ...
    if (value !== '') {
        const element = data![level.length && level[0]];
        if (element) {
