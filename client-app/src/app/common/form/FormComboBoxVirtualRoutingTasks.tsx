...
export const FormComboBoxVirtualRoutingTasks = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    const textField = "workEffortName";
    const keyField = "workEffortIdTo";
    const emptyItem: Item = {[textField]: "loading ...", workEffortIdTo: "0"};
    const pageSize = 10;
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        agent.WorkEfforts.getRoutingTasksLov(params)
            .then((json) => {
                if (json) {
                    const total = json.routingTaskCount;
                    const items: Item[] = [];
                    json.routingTasks.forEach((element: any, index: any) => {
                        const {workEffortIdTo, workEffortName} = element;
                        const item: Item = {
                            [keyField]: workEffortIdTo,
                            [textField]: workEffortName,
                        };
                        items.push(item);
