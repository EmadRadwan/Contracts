...
export const FormSimpleComboBoxRawMaterialVirtual = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    const requestData = React.useCallback(
        (skip: number, filter: string) => {
            ...
            // REFACTOR: Use specific endpoint for RAW_MATERIAL
            // Purpose: Directly fetch RAW_MATERIAL products without certificateType
            // Context: Eliminates dependency on certificateType
            agent.Products.getRawMaterialProductsLov(params)
                .then((json) => {
                    if (json) {
