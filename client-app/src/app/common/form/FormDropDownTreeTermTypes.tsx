...
export const FormDropDownTreeTermTypes = (fieldRenderProps: FieldRenderProps & DropDownTreeProps) => {
    const {
        validationMessage,
        touched,
        label,
    ...
    function searchTree(element: any, matchingTermTypeId: any): any {
        if (!matchingTermTypeId) return false
        if (element.termTypeId === matchingTermTypeId) {
            return element;
        } else if (element.items != null) {
            let i;
            let result = null;
            for (i = 0; result == null && i < element.items.length; i++) {
                result = searchTree(element.items[i], matchingTermTypeId);
            }
            return result;
        }
        return null;
