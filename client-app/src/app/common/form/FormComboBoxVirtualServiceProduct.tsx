...
export const FormComboBoxVirtualServiceProduct = (fieldRenderProps: FieldRenderProps) => {
    const {
        validationMessage,
        touched,
        onFocus,
    ...
    const requestData = React.useCallback((skip: number, filter: string) => {
        ...
        agent.Products.getServiceLov(params)
            .then((json) => {
                if (json) {
                    const total = json.servicesCount;
                    const items: Item[] = [];
                    json.serviceProducts.forEach((element: any, index: any) => {
