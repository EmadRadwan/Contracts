...
-- of 20,000.
--
-- VERIFIED (2026-08-04): INV1376 is still INVOICE_IN_PROCESS with zero
-- ACCTG_TRANS and zero PAYMENT_APPLICATION -- not posted or paid, safe to
-- correct outright. Order item 0002 is ITEM_COMPLETED with one
-- ORDER_ITEM_BILLING row (to INV1376) and 3 ORDER_STATUS history rows, no
-- ORDER_ADJUSTMENT rows.
--
-- FIX: remove the redundant deduction order item (0002) and its billing/
-- invoice-item/status rows entirely, leaving PO11071 with just its correct
-- order item 0001 and INV1376 with just its correct 20,000 invoice item.
--
-- REVIEW BEFORE RUNNING. Take a backup first:
--   mysqldump erp_contracts INVOICE INVOICE_ITEM INVOICE_STATUS INVOICE_ROLE \
--     ORDER_ITEM ORDER_ITEM_BILLING ORDER_STATUS > backup_before_82_0006_dedup.sql
-- ============================================================================
...
-- status history, and the order item itself. Does not touch order item
-- 0001, INVOICE_ITEM 00001, or the WORK_EFFORT certificate item -- those are
-- correct as-is.
