...
--
-- VERIFIED (2026-08-04, against a production data mirror): AFTER state came
-- out to exactly 18 items totaling 79,340.13 -- matches the user-reported
-- pre-duplication baseline (19 items / 79,340) almost to the cent, off by
-- one item count only (18 vs 19, likely a minor miscount in the original
-- report, not a missing item -- the total reconciles exactly).
--
...
SELECT 'AFTER' phase,
       ...
       (SELECT ROUND(SUM(QUANTITY*UNIT_PRICE),2) FROM ORDER_ITEM WHERE ORDER_ID='PO11075') order_total_after_MUST_BE_79340_13;
