...
END AS RecordType,
    ...
    -- CertificateType and Arabic version
    CASE
...
FROM WORK_EFFORT header
         ...
         -- Project join (projects live in the same table)
         LEFT JOIN WORK_EFFORT project 
                    ON COALESCE(header.PROJECT_ID, item.PROJECT_ID) = project.WORK_EFFORT_ID
                       AND project.WORK_EFFORT_TYPE_ID = 'PROJECT'   -- optional but recommended for safety
