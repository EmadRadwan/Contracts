...
GRANT SELECT ON Fact_NonProject_Commitments TO 'powerbi_user'@'%'; -- added 2026-08-18 alongside the semantic model's Fact_NonProject_Commitments table. Without it Power Query fails with "The key didn't match any rows in the table" — the view is invisible to powerbi_user, so the Source{[Schema=..,Item=..]} navigation finds nothing. Run this line alone; re-running the whole script DROPs and recreates the user.
