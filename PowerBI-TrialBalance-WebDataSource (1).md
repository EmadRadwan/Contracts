...
| `400 Bad Request` on `getCurrentTimePeriod` | New endpoint not present in the **deployed** build, and the old route shape is swallowing the request. See section 5. |

---

## 5. Incident: `400 Bad Request` on `getCurrentTimePeriod` (2026-07-21)

### Symptom

Refreshing `Fact_TrialBalance` in Desktop fails with:

```
Web.Contents failed to get contents from
'http://129.146.22.240:5100/api/CustomTimePeriods/6010/getCurrentTimePeriod' (400): Bad Request
```

### What the status code rules out

A `400` is a response **from the API server**, which means the host was reachable, the port
was open, the Anonymous credential was accepted, and the Formula Firewall did not block the
query. It also means the failure is on the **first** of the two chained web calls — the
`generateTrialBalanceReport` call never ran.

So this is *not* a Power BI / M problem. The M query in section 3 is correct as written.

### Root cause

The endpoint does not exist in the build running on `129.146.22.240:5100`.

Per section "Status: uncommitted" and section 5 above, both
`API/Controllers/Accounting/CustomTimePeriodsController.cs` (modified) and
`Application/Accounting/OrganizationGlSettings/GetCurrentTimePeriod.cs` (added) are
**uncommitted local changes that were never built or deployed**. `129.146.22.240` is a
separately deployed instance running an older build, so it has never seen the route
`GET api/CustomTimePeriods/{organizationPartyId}/getCurrentTimePeriod`.

### Why 400 and not 404

Because the route *shape* still matches a pre-existing action. `CustomTimePeriodsController`
has an existing two-segment GET (e.g. `[HttpGet("{organizationPartyId}/{customTimePeriodId}")]`).
ASP.NET Core matches `6010/getCurrentTimePeriod` against it, model-binds the literal string
`getCurrentTimePeriod` as the period id, and the binder or handler rejects it → `400`.
"Route matched, value invalid" produces 400; a genuinely unmatched route produces 404.

**This can persist after deployment.** If the old route is registered before the new one it
will keep shadowing it. If the 400 survives a deploy, disambiguate — constrain the old
route's id parameter (`{customTimePeriodId:int}`, or a regex constraint) or reorder/rename
so the literal `getCurrentTimePeriod` segment can't be bound as an id.

### Diagnostic — surfacing the real response body

Power Query hides 4xx response bodies by default, so the Desktop error shows only the bare
status code. Paste this into a blank query to see what the server actually said:

```m
let
    r = Web.Contents("http://129.146.22.240:5100",
        [RelativePath = "api/CustomTimePeriods/6010/getCurrentTimePeriod",
         ManualStatusHandling = {400, 404, 500}]),
    body = Text.FromBinary(r),
    status = Value.Metadata(r)[Response.Status]
in
    [Status = status, Body = body]
```

A model-binding complaint confirms the route-collision reading; a "no route matched" page
means the request isn't hitting the controller at all.

### Resolution steps

1. Confirm what is actually deployed — open `http://129.146.22.240:5100/swagger/index.html`
   and search for `getCurrentTimePeriod`. Absent ⇒ diagnosis confirmed.
2. Build, commit, and deploy the backend changes to that host.
3. Re-refresh `Fact_TrialBalance` in Desktop.
4. If the 400 survives the deploy, apply the route-disambiguation fix described above.

### Verified state at time of writing

Live Desktop session (`Projects-21-Jul`, Tabular port 60383) inspected via XMLA:
`Fact_TrialBalance` exists with its M partition intact (`sourceType: M`, `mode: Import`,
`state: NoData`). Model-side setup is correct; nothing in the semantic model needs changing.
