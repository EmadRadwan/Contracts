...
public class SeedMedications
{
        ...
        UserManager<AppUserLogin> userManager, RoleManager<ApplicationRole> roleManager)
    {
        ...
        var productIds = context.Products.Select(s => s.ProductId)
            .ToList();
        ...
        if (!context.Parties.Any())
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                var fakeParty = new Faker<Party>()
                    .RuleFor(o => o.PartyId, f => partyId)
                    .RuleFor(o => o.PartyTypeId, "PERSON")
                    .RuleFor(o => o.MainRole, "EMPLOYEE")
                    .RuleFor(o => o.StatusId, "PARTY_ENABLED")
                    .RuleFor(o => o.Description, f => firstName + " " + lastName)
                    .RuleFor(o => o.CreatedDate, f => nowDateTime)
                    .RuleFor(o => o.CreatedStamp, f => nowDateTime)
                    .RuleFor(o => o.LastUpdatedStamp, f => nowDateTime);
                ...
                var randomRoleType = roleTypes1[new Random().Next(roleTypes1.Length)];
                var fakePartyRole = new Faker<PartyRole>()
                    .RuleFor(o => o.PartyId, f => partyId)
                    .RuleFor(o => o.RoleTypeId, f => randomRoleType)
                    .RuleFor(o => o.CreatedStamp, f => nowDateTime)
                    .RuleFor(o => o.LastUpdatedStamp, f => nowDateTime);
        ...
        // get all parties from the database where main role is customer
        var customers = await context.Parties
            .Where(x => x.MainRole == "CUSTOMER")
            .ToListAsync();
        // loop for each customer and select from PartyContactMech where partyId = customer.PartyId
        // also join with ContactMech and select ContactMech that has ContactMechTypeId = 'TELECOM_NUMBER'
        // if we have a record then we will select the BillingAccount from the database where partyId = customer.PartyId
        // where the BillingAccount has A BillingAccountRole - join BillingAccount BillingAccountRole - BillingAccountRole has PartyId = customer.PartyId
        // THEN Update the BillingAccount with the ContactMechId.
        foreach (var customer in customers)
        {
            var partyContactMech = await context.PartyContactMeches
                .Where(x => x.PartyId == customer.PartyId)
                .Include(x => x.ContactMech)
                .Where(x => x.ContactMech.ContactMechTypeId == "POSTAL_ADDRESS")
                .SingleOrDefaultAsync();

            if (partyContactMech != null)
            {
                var billingAccount = await context.BillingAccounts
                    .Include(x => x.BillingAccountRoles)
                    .Where(x => x.BillingAccountRoles.Any(x => x.PartyId == customer.PartyId))
                    .SingleOrDefaultAsync();
                billingAccount.ContactMechId = partyContactMech.ContactMechId;
            }
        }
        ...
        */
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workefforts.json");
            var jsonData = File.ReadAllText(path);
        ...
        if (!context.WorkEffortPartyAssignments.Any())
        {
            ...
            foreach (var partyRole in partyRoles2)
            {
                // Select a random routing task for each employee
                var routingTask = routingTasks[random.Next(routingTasks.Count)];
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/cost_components.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workeffort_assocs.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workeffort_good_standards.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/workeffort_cost_calcs.json");
            var jsonData = File.ReadAllText(path);
