// BillingAccountForm.tsx – FINAL VERSION (Handles 200 + isSuccess: false correctly)
...
const BillingAccountForm: React.FC<Props> = ({
    ...
    const [create, { isLoading: isCreating }] = useCreateBillingAccountMutation();
    ...
    // Trigger balance fetch only when account exists (edit or just created)
    useEffect(() => {
    ...
    return (
        <Paper elevation={5} className="div-container-withBorderCurved" sx={{ p: 3 }}>
            ...
            {/* Show error from backend (200 + isSuccess: false) */}
            {creationError && (
                <Alert severity="error" sx={{ mb: 3 }} onClose={() => setCreationError(null)}>
                    {creationError}
                </Alert>
            )}
                    ...
                    return (
                        <FormElement>
                            <fieldset className="k-form-fieldset">
                                ...
                                {/* Balance Box – Only after account exists */}
                                {showBalanceBox && (
