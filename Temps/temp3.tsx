...
export default function EditMultiAcctgTrans() {
    ...
    const companyId = useAppSelector(
        (state: RootState) => state.accountingSharedUi.selectedAccountingCompanyId
    );

    // ────────────────────────────────────────────────
    // Single source of truth: ID from URL params
    // This survives navigation (including after duplicate)
    const { acctgTransId: currentTransId } = useParams<{ acctgTransId: string }>();

    // Optional: keep location.state only for INITIAL population
    const location = useLocation();
    const initialTransFromState = location.state?.selectedAcctgTrans;

    // ────────────────────────────────────────────────
    // Fetch entries using the URL param (reliable)
    const { data: transEntriesData, isLoading: isLoadingEntries } =
        useFetchGeneralAcctTransEntriesQuery(currentTransId!, {
            skip: !currentTransId,
        });
    ...
    const { isLoading: isUpdating, handleUpdateMultiAcctgTransWithEntries } =
        useEditMultiAcctgTrans();
    ...
    // ────────────────────────────────────────────────
    // Header values – initialize only once (on mount)
    // We use location.state only for the very first render
    // After navigation (duplicate), user will see fetched/current data
                        ...
                        ? [
                            {
                                ...
                                acctgTransEntrySeqId: undefined,
                            },
                        ]
                        : []),
                    ...(creditGlAccountId
        ...
        try {
            await handleUpdateMultiAcctgTransWithEntries({
                ...
                UpdateMultiAcctgTransParams: {
                    ...
                    IsPosted: justPosted ? "Y" : "N", // ← better than relying on stale state
    ...
    const GlAccountCell = useCallback(
        ({ dataItem }: GridCellProps) => {
            ...
            const transEntry = transEntriesData?.find((e) => e.glAccountId === glAccountId);
    ...
    const isLoading = isUpdating || isPosting || isLoadingEntries;
