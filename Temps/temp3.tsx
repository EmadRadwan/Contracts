...
interface PaymentPlanModalProps {
    ...
    onApply: (installments: Array<{ dueDate: string; amount: number; isAdvance: boolean }>) => void;
    isPreview?: boolean;
    initialInstallments?: Array<{ dueDate: string; amount: number; isAdvance: boolean }>;
}
...
export default function PaymentPlanModal({
    ...
    // Validation
    const validateRow = (row: InstallmentRow) => {
        let dateError = "";
        let amountError = "";
    ...
    const handleSplitChange = (count: number) => {
        ...
        const equalAmount = advancePayment / count;

        if (diff > 0) {
            const updatedCurrent = currentAdvanceRows.map((row) => ({ ...row, amount: equalAmount }));
            ...
            setRows([...updatedCurrent, ...newAdvanceRows, ...regularRows]);
        } else {
            const keptAdvanceRows = currentAdvanceRows.slice(0, count).map((row) => ({
                ...row,
                amount: equalAmount,
            }));
            setRows([...keptAdvanceRows, ...regularRows]);
        }
    };
    ...
    const deleteAdvanceRow = (id: string) => {
        ...
        setRows((prev) => {
            const remainingAdvance = prev.filter((r) => r.isAdvance && r.id !== id);
    ...
    const saveEdit = () => {
        ...
        setRows((prev) => {
            ...
            if (isEditingAdvance) {
                const newAdvanceSum = newRows.filter((r) => r.isAdvance).reduce((s, r) => s + r.amount, 0);
                ...
                if (regularCount > 0) {
                    ...
                    if (Math.abs(accumulatedRoundingError) > 0.001) {
                        ...
                        if (lastRegularIdx !== -1) {
                            ...
                            newRows[lastRegularIdx].amount = Math.round(newRows[lastRegularIdx].amount * 100) / 100;
                        }
                    }
                }
            }
        ...
        <Grid container padding={2} spacing={2}>
            ...
            {advancePayment > 0 && (
                <Grid item xs={12}>
                    <Box display="flex" alignItems="center" gap={2}>
                        ...
                        {advanceSplitCount < 3 && (
                            <IconButton color="primary" onClick={() => handleSplitChange(advanceSplitCount + 1)}>
                                <AddIcon />
                            </IconButton>
                        )}
                    </Box>
            ...
            <Grid item xs={12}>
                <div style={{ height: "400px", overflow: "auto" }}>
                    <KendoGrid data={rows} scrollable="none">
                        <Column
                            field="number"
                            title="#"
                            width="80"
                            cell={(props) => (
                                <td>
                                    {props.dataItem.isAdvance
                                        ? `A${props.dataItem.number}`
