...
export default function PaymentPlanModal({
    ...
    // ==================== REFACTORED: Stable Memoized Cell Component ====================
    // This prevents flashing by keeping the same component reference across renders
    const ActionsCell = useMemo(() => {
        const CellComponent = React.memo(({ dataItem }: { dataItem: InstallmentRow }) => {
            return (
    ...
    // ==================== REFACTORED: Stable Selection Callbacks ====================
    ...
    return (
        <Grid container padding={2} spacing={2}>
                ...
                <Box display="flex" alignItems="center" flexWrap="wrap" gap={3}>
                    {advancePayment > 0 && (
                        <Box display="flex" alignItems="center" gap={2}>
                            ...
                            {advanceSplitCount < 3 && (
                                <IconButton color="primary" onClick={addAdvanceRow} sx={{ p: 1 }}>
                                    <AddIcon />
                                </IconButton>
                            )}
                        </Box>
            ...
            <Grid item xs={12}>
                <div style={{ height: '400px', overflow: 'auto' }}>
                    ...
                    >
                        ...
                        {/* REFACTORED: Using stable memoized component instead of inline function */}
                        <Column
                            title={getTranslatedLabel("salesRequest.paymentPlan.actions", "Actions")}
                            width="140"
                            cell={ActionsCell}
                        />
                    </KendoGrid>
            ...
            {/* Summary & Actions sections remain the same */}
            <Grid item xs={12}>
            ...
            {/* Edit Modal, Bulk Edit Dialog, Recreate Confirm Dialog remain unchanged */}
            <Dialog open={!!editModal} onClose={() => setEditModal(null)} maxWidth="sm" fullWidth>
                ...
                <DialogActions>
                    <Button onClick={() => { setEditModal(null); setDateError(""); setAmountError(""); }}>
            ...
            {/* Bulk Edit & Recreate dialogs remain the same... */}
            {/* (Omitted for brevity - they didn't change) */}
        </Grid>
    );
}