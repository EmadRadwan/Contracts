...
public class SeedCars
{
        ...
        UserManager<AppUserLogin> userManager, RoleManager<ApplicationRole> roleManager)
    {
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_category_types_Cars.json");
            var jsonData = File.ReadAllText(path);
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/product_categories_Cars.json");
            var jsonData = File.ReadAllText(path);
        ...
        var sparePartParentCategories = context.ProductCategories
            .Where(w => w.PrimaryParentCategoryId == "SPARE_PARTS")
            .ToList();

        // create spare parts categories list where parent category is in sparePartParentCategories
        var sparePartCategories = new List<ProductCategory>();
        foreach (var sparePartParentCategory in sparePartParentCategories)
        {
            var sparePartCategory = context.ProductCategories
                .Where(w => w.PrimaryParentCategoryId == sparePartParentCategory.ProductCategoryId)
                .ToList();
            sparePartCategories.AddRange(sparePartCategory);
        }

        var vehicleServiceParentCategories = context.ProductCategories
            .Where(w => w.PrimaryParentCategoryId == "VEHICLE_SERVICES")
            .ToList();

        // create vehicle services categories list where parent category is in vehicleServiceParentCategories
        var vehicleServiceCategories = new List<ProductCategory>();
        foreach (var vehicleServiceParentCategory in vehicleServiceParentCategories)
        {
            var vehicleServiceCategory = context.ProductCategories
                .Where(w => w.PrimaryParentCategoryId == vehicleServiceParentCategory.ProductCategoryId)
                .ToList();
            vehicleServiceCategories.AddRange(vehicleServiceCategory);
        }
        ...
        var productIds = context.Products.Where(x => x.ProductTypeId == "FINISHED_GOOD").Select(s => s.ProductId)
            .ToList();
        ...
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Json/tax_authority_categories_Cars.json");
            var jsonData = File.ReadAllText(path);
