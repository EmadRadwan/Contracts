...
using Serilog;
using Serilog.Enrichers.ExceptionDetails;
using Serilog.Events;
using Serilog.Sinks.SystemConsole.Themes;

var logDirectory = Environment.GetEnvironmentVariable("SERILOG_LOG_PATH") ?? "logs";
// REFACTOR: Configure Serilog to focus on Warning and Error logs with detailed context
// Changed file sink to Warning level to reduce noise from Information logs
// Added ExceptionDetails enricher for detailed stack traces and error context
// Added RequestId enricher for correlating logs with HTTP requests
Log.Logger = new LoggerConfiguration()
    .Enrich.WithThreadId()
    .Enrich.WithMachineName()
    .Enrich.WithExceptionDetails() // Adds detailed exception data (requires Serilog.Enrichers.ExceptionDetails)
    .Enrich.WithRequestId() // Adds request correlation ID (requires Serilog.Enrichers.RequestId)
    .WriteTo.Console(
...
app.UseEndpoints(endpoints =>
{
    ...
    endpoints.Map("/api/{**path}", async context =>
    {
        ...
        logger.LogWarning("Unmatched API endpoint requested: {Method} {Path}", context.Request.Method, context.Request.Path);
